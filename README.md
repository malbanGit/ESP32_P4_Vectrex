...
| USB | HID keyboard + USB HID joystick (hot-plug, full descriptor parsing) |
...
### Device reading priority

Three input sources are available. The priority order is declared explicitly in `defines.h` with the comment:
...
```
1. 9-pin ADC joystick (is9PinJoystickAvailable / get9PinAnalogX / get9PinAnalogY)
2. USB HID joystick   (isUSBJoystickAvailable  / getUSBAnalogX  / getUSBAnalogY)
3. Keyboard fallback  (g_inputState / isKeyDown / isAsciiDown)
```

A typical input read looks like this:
...
// else: g_inputState is already populated from keyboard by readevents()
```

`readevents()` — called automatically inside `mini_end_frame()` — populates `g_inputState` from all available sources. If a physical joystick of any kind is connected, keyboard port-0 input is suppressed; port-1 always uses the keyboard.
...
The USB HID keyboard state is polled once per frame inside `mini_end_frame()`. Do not spin a separate polling task.
...
```c
bool is9PinJoystickAvailable(void); // true if a physical analog joystick is detected
int  get9PinAnalogX(void);          // X axis: -128 … +127, 0 = center
int  get9PinAnalogY(void);          // Y axis: -128 … +127, 0 = center
```
...
Any standard USB HID joystick or gamepad is supported. The driver parses the device's HID report descriptor at connect time, automatically locating the X and Y axes regardless of vendor-specific report layouts. Buttons are mapped to `g_inputState.buttonState`.

```c
bool isUSBJoystickAvailable(void); // true while a USB joystick is connected
int  getUSBAnalogX(void);          // X axis: -128 … +127, 0 = center
int  getUSBAnalogY(void);          // Y axis: -128 … +127, 0 = center
```

**Hot-plug:** the USB host stack is event-driven. Plug a joystick in while the system is running and it is recognised automatically; unplug it and `isUSBJoystickAvailable()` returns `false` immediately. No restart required.

The driver handles signed and unsigned axis encodings, any bit-width, and report descriptors that use multiple Report IDs. Axis values are sign-extended and scaled to the −128…+127 range. If the device turns out not to have X/Y axes it is released and the driver continues looking for a keyboard.
...
### Input — 9-pin analog joystick

```c
bool is9PinJoystickAvailable(void);
// Returns true when a physical analog joystick is detected on the 9-pin connector.

int get9PinAnalogX(void);
// X axis: -128 (full left) … 0 (center) … +127 (full right).
// Dead zone applied around center to suppress jitter.

int get9PinAnalogY(void);
// Y axis: -128 … 0 … +127. Same dead zone behaviour.
```

### Input — USB HID joystick

```c
bool isUSBJoystickAvailable(void);
// Returns true while a USB HID joystick or gamepad is connected.
// Automatically becomes false when the device is unplugged (hot-plug aware).

int getUSBAnalogX(void);
// X axis: -128 (full left) … 0 (center) … +127 (full right).
// Scaled from the device's logical axis range reported in its HID descriptor.
