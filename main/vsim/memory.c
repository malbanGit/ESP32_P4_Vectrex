...
byte MEMRD(unsigned addr, int PC, unsigned long cyc)
{
  ...
  switch(tag & 0x7f) 
    ...
    case MBSTAT:
      ...
      break;
    case GRAVITAR_IN1:
      result = ((optionreg [2] & 0x07) << 5) | 	((! switches [0].thrust) << 4) | 	((! switches [0].left) << 3) | 	((! switches [0].right) << 2) |
                ((! switches [0].fire) << 1) |  ((! switches [0].shield));
      break;
    case GRAVITAR_IN2:
      result = 0x80 | /* upright cabinet */ ((! check_switch_decr (& start2)) << 6) | 	((! check_switch_decr (& start1)) << 5) | 	((! switches [1].thrust) << 4) |
	              ((! switches [1].left) << 3) | ((! switches [1].right) << 2) |  ((! switches [1].fire) << 1) | 	((! switches [1].shield));
      break;
    case SD_INPUTS:
      switch (addr & 0x07)
      {
      ...
      break;
    case BZ_INPUTS:
      result = 	(check_switch_decr (& start1) << 5) | 	(switches [0].fire << 4) | 	(switches [0].leftfwd << 3) | 	(switches [0].leftrev << 2) |
	                (switches [0].rightfwd << 1) |(switches [0].rightrev << 0);
      break;
    case LUNAR_MEM:
