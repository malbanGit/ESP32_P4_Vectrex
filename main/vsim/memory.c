...
//        switches [0].abort =0;
	...
	{
	case 0: // left coin 
	  result = !(check_switch_decr (& cslot_left) );
	  break;
	case 1: // center coin 
	  result = !(check_switch_decr (& cslot_util) );
	  break;
	case 2: // right coin 
	  result = !(check_switch_decr (& cslot_right));
	  break;
...
/*
byte MEMRD_org(unsigned addr, int PC, unsigned long cyc)
{
  register byte tag,result=0;
  ...
  switch(tag & 0x7f) 
	...
	case 6:
	  return (((check_switch_decr (& start2)) << 7) |
		  ((optionreg [2] << 5) & 0x40));
	case 7:
	  return (0x00 | ((optionreg [2] << 4) & 0x40));
	}
      result = 0x00;
      break;
    case BZ_INPUTS:
