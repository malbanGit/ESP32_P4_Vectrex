    ...
    for (int py = by0; py <= by1; py++) {
        ...
        for (int px = bx0; px <= bx1; px++) {
            ...
            /* ── Distance-field contribution ───────────────────────────── */
            {
                ...
                /* Additive Y + chroma delta for this pixel.
                 * For odd-column pixels the pair's U/V bytes are at dst[-1]/dst[+1];
                 * for even-column pixels they are at dst[+1]/dst[+3].
                 * Always update both so vertical lines get correct colour. */
                ...
                {
                    int u_off = (px & 1) ? -1 : 1; /* U offset from dst[0] */
    ...
    if (s_overlay_pal) {
        ...
        for (int py = by0; py <= by1; py++) {
            ...
            for (int px = bx0; px <= bx1; px++) {
                ...
                /* Restore pair's U/V: even px → dst[+1,+3]; odd px → dst[-1,+1]. */
                {
                    int u_off = (px & 1) ? -1 : 1;
